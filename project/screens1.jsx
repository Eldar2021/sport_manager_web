...
// ─── Login ────────────────────────────────────────────────────
function ScreenLogin({ theme, go, role }) {
  ...
  return (
    <div data-screen-label="02 Login" style={{ height: '100%', background: theme.bg, display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16, flex: 1 }}>
        ...
        />
        <button onClick={() => go('resetPassword')} style={{ background:'none', border:'none', color: theme.primary, fontFamily:'Inter', fontSize: 14, fontWeight: 600, cursor:'pointer', alignSelf:'flex-start', padding: 0 }}>
          {RU.forgotPassword}
        </button>
      </div>
      <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <div style={{ textAlign: 'center', fontFamily: 'Inter', fontSize: 15, color: theme.ink500 }}>
          {RU.noAccount} <button onClick={()=>go(role === 'manager' ? 'registerManager' : 'role')} style={{ background:'none', border:'none', color: theme.primary, fontWeight:600, cursor:'pointer', padding:0, fontSize: 15 }}>{RU.register}</button>
        </div>
      </div>
    </div>
  );
...
// ─── Home ─────────────────────────────────────────────────────
function ScreenHome({ theme, go, role, tables, setTables, currentVenue, now, openVenuePicker, setActiveTableId, hasVenue, setHasVenue }) {
  ...
  // Owner empty-venue state: no venues yet
  if (role === 'owner' && hasVenue === false) {
    return (
      <div data-screen-label="05 Home — No Venue" style={{ height: '100%', background: theme.bg, display: 'flex', flexDirection: 'column' }}>
        <div style={{
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 16, paddingBottom: 24, position: 'relative' }}>
        ...
        {loading ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '40px 20px', textAlign: 'center', gap: 16 }}>
            ...
            {role === 'owner' && (
              <TFButton theme={theme} kind="primary" size="md" style={{ maxWidth: 220 }} onClick={() => { if (window.setEditTable) window.setEditTable(null); go('tableDetail'); }}>
...
// ─── First Venue Setup (owner onboarding) / Venue Edit ────────
function ScreenFirstVenueSetup({ theme, go, editVenue, setVenues }) {
  const isEdit = !!editVenue;
  const [name, setName] = React.useState(editVenue?.name || '');
  const [code, setCode] = React.useState(editVenue?.code || '');
  const [step, setStep] = React.useState(1); // 1 = venue info, 2 = success (create only)
  ...
  return (
    <div data-screen-label={isEdit ? '11b Venue Edit' : '05 First Venue Setup'} style={{
      ...
      <div style={{
        ...
        <button onClick={() => go(isEdit ? 'venueMgmt' : 'registerOwner')} style={{ background: 'none', border: 'none', color: theme.primary, padding: 8, cursor: 'pointer' }}><Icon.back/></button>
