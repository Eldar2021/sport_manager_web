...
// ─── Venue Management ─────────────────────────────────────────
function ScreenVenueMgmt({ theme, go, venues, setVenues, setEditVenue, setActiveVenueId }) {
  return (
    <div data-screen-label="11 Venue Mgmt" style={{ height: '100%', background: theme.bg, display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      <div style={{ flex: 1, overflow: 'auto', padding: 16 }}>
        {venues.length === 0 ? (
          <div style={{
            ...
            <div style={{ fontFamily: 'Inter', fontSize: 20, fontWeight: 800, color: theme.ink900, letterSpacing: -0.4 }}>
              Залов пока нет
            </div>
...
// ─── Venue Detail (info + tables + 3-dot menu) ─────────────────
function ScreenVenueDetail({ theme, go, venue, tables, setEditVenue, setShowDeleteVenue, setEditTable }) {
  const [menuOpen, setMenuOpen] = React.useState(false);
  if (!venue) return null;
  // Venue → tables: in this prototype we don't have a venue_id on each table; show all tables as belonging to venue.
  // Use tables from props (live state).
  const venueTables = tables || [];

  return (
    <div data-screen-label="11a Venue Detail" style={{
      height: '100%', background: theme.bg, display: 'flex', flexDirection: 'column', position: 'relative',
    }}>
      <SimpleHeader theme={theme} onBack={() => go('venueMgmt')} title={venue.name}
        right={
          <button onClick={() => setMenuOpen(o => !o)} style={{
            background: 'transparent', border: 'none', padding: 8, cursor: 'pointer', color: theme.ink700,
          }}><Icon.more/></button>
        }
      />
      ...
      <div style={{ flex: 1, overflow: 'auto' }}>
        ...
        <div style={{ padding: 16 }}>
          <div style={{
            ...
            <div style={{
              width: 56, height: 56, borderRadius: 14, background: theme.primaryLight,
              color: theme.primary, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
            }}><Icon.pin/></div>
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontFamily: 'Inter', fontSize: 18, fontWeight: 800, color: theme.ink900, letterSpacing: -0.3 }}>
                {venue.name}
              </div>
        ...
        <div style={{ padding: '4px 16px 16px' }}>
          <div style={{
            ...
            <div style={{ fontFamily: 'Inter', fontSize: 12, fontWeight: 700, color: theme.ink500, textTransform: 'uppercase', letterSpacing: 0.5 }}>
              Столы зала
            </div>
            <div style={{ fontFamily: 'Inter', fontSize: 12, color: theme.ink500 }}>
              {venueTables.length} шт.
            </div>
          </div>
...
function ScreenResetPassword({ theme, go }) {
  ...
  const contactSheet = contactOpen && (
    <>
      ...
      <div style={{
        ...
        {[
          { label: 'WhatsApp', sub: '+996 555 12-34-56', bg: '#25D36615', fg: '#25D366', icon: (
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M20.5 3.5A10 10 0 003.6 17.3L2 22l4.9-1.5a10 10 0 0013.6-17zM12 20.2a8.3 8.3 0 01-4.2-1.1l-.3-.2-2.9.9.9-2.8-.2-.3a8.3 8.3 0 1111.6 2.2 8.3 8.3 0 01-4.9 1.3zm4.8-6.2c-.3-.1-1.6-.8-1.8-.9-.3-.1-.4-.1-.6.1-.2.3-.7.9-.8 1-.2.2-.3.2-.6.1-.3-.1-1.1-.4-2.1-1.3-.8-.7-1.3-1.5-1.5-1.8-.2-.3 0-.4.1-.6l.4-.5c.1-.2.2-.3.3-.5.1-.2 0-.4 0-.5-.1-.1-.6-1.4-.8-1.9-.2-.5-.4-.4-.6-.4h-.5c-.2 0-.5.1-.7.3-.2.3-1 .9-1 2.3s1 2.7 1.2 2.9c.1.2 2 3.1 5 4.3.7.3 1.2.5 1.6.6.7.2 1.3.2 1.8.1.6-.1 1.6-.7 1.9-1.3.2-.7.2-1.2.2-1.3-.1-.1-.3-.2-.6-.3z" fill="currentColor"/></svg>
          ) },
          { label: 'Telegram', sub: '@ymyrkai_support', bg: '#229ED915', fg: '#229ED9', icon: (
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M21.5 3.2L2.9 10.4c-.9.4-.9 1.6 0 1.9l4.2 1.3 1.6 5.1c.2.7 1 .9 1.5.4l2.3-2.1 4.4 3.2c.6.4 1.4.1 1.6-.6l3.4-14.7c.2-.9-.7-1.7-1.4-1.7zm-3 4.5l-8.2 7.4-.3 3.1-1.3-4.2 9.8-6.3z" fill="currentColor"/></svg>
          ) },
          { label: 'Email', sub: 'support@ymyrkai.kg', bg: theme.primaryLight, fg: theme.primary, icon: (
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><rect x="3" y="5" width="18" height="14" rx="2" stroke="currentColor" strokeWidth="1.8"/><path d="M3 7l9 6 9-6" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/></svg>
          ) },
          { label: 'Позвонить', sub: '+996 700 12-34-56', bg: theme.successLight, fg: theme.success, icon: (
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none"><path d="M22 16.9v3a2 2 0 01-2.2 2 19.8 19.8 0 01-8.6-3.1 19.5 19.5 0 01-6-6A19.8 19.8 0 012.1 4.2 2 2 0 014.1 2h3a2 2 0 012 1.7 12.8 12.8 0 00.7 2.8 2 2 0 01-.5 2.1L8 9.9a16 16 0 006 6l1.3-1.3a2 2 0 012.1-.5 12.8 12.8 0 002.8.7 2 2 0 011.8 2z" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round"/></svg>
          ) },
        ].map((c, i, arr) => (
  ...
  if (sent) return (
    <div data-screen-label="16 Reset Password — Sent" style={{ height: '100%', background: theme.bg, display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 32, textAlign: 'center' }}>
        ...
        <div style={{ fontFamily: 'Inter', fontSize: 24, fontWeight: 800, color: theme.ink900, letterSpacing: -0.4 }}>
        ...
        <div style={{ marginTop: 28, width: '100%', maxWidth: 320, display: 'flex', flexDirection: 'column', gap: 10 }}>
          ...
          <button onClick={() => setContactOpen(true)} style={{
            ...
            fontFamily: 'Inter', fontSize: 14, fontWeight: 600, color: theme.primary,
          }}>
  ...
  return (
    <div data-screen-label="16 Reset Password" style={{ height: '100%', background: theme.bg, display: 'flex', flexDirection: 'column', position: 'relative' }}>
      ...
      <div style={{ flex: 1, padding: 24, display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        <TFInput theme={theme} label="Логин или Email" placeholder="например, azamat@mail.kg"
          value={loginOrEmail} onChange={(e) => setLoginOrEmail(e.target.value)}/>

        <button onClick={() => setContactOpen(true)} style={{
          marginTop: 4,
          background: 'transparent', border: `1.5px dashed ${theme.ink300}`,
          borderRadius: 12, padding: '14px 16px', cursor: 'pointer',
          display: 'flex', alignItems: 'center', gap: 12, textAlign: 'left',
        }}>
          <div style={{
            width: 36, height: 36, borderRadius: 10, background: theme.primaryLight, color: theme.primary,
            display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0,
          }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontFamily: 'Inter', fontSize: 14, fontWeight: 700, color: theme.ink900 }}>
              Не приходит ссылка?
            </div>
            <div style={{ fontFamily: 'Inter', fontSize: 12, color: theme.ink500, marginTop: 2 }}>
              Свяжитесь с нами — WhatsApp, Telegram, email, звонок
            </div>
          </div>
          <span style={{ color: theme.ink300 }}><Icon.chevron/></span>
        </button>
      ...
      <div style={{ padding: 24 }}>
        <TFButton theme={theme} disabled={!valid} onClick={() => setSent(true)}>
          Отправить ссылку
        </TFButton>
      </div>
      {contactSheet}
...
Object.assign(window, {
  ScreenSettings, ScreenVenueMgmt, ScreenVenueDetail, ScreenTableMgmt, ScreenTableDetail,
